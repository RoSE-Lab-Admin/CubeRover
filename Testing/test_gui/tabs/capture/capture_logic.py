...
async def run_test_engine(profile_filename, client):
    try:
        ...
        if not df.empty:
            ...
            with client:
                if final_report['passed']: ui.notify("✅ Test Passed!", type=COLORS['positive'])
                else: ui.notify("❌ Test Failed! Check console.", type=COLORS['negative'])
        else:
            with client: ui.notify("Cannot evaluate: No data was captured.", type=COLORS['warning'])
            ...
            for chart_id, col_name, color, dash, is_visible in metrics_to_load:
                if col_name in state.live_reference_df.columns:
                    ...
                    UI.master_chart_card.add_series({
                        ...
                        "color": color, 
                        "dashStyle": dash, "opacity": 0.4, "visible": is_visible, "marker": {"enabled": False}
                    })
