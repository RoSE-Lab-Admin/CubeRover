...
class MultiPlot(Node):
    ...
    def mocap_callback(self, msg):
        with self.mocaplock:
            ...
            self.mocap_data['pose'][1].append(msg.pose.position.y)
            self.mocap_data['pose'][2].append(msg.pose.position.z)  
            print(f"{len(self.mocap_data['time'])}; {len(self.mocap_data['pose'][0])}")
