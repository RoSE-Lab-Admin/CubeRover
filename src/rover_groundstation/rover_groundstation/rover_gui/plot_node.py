...
class MultiPlot(Node):
    def __init__(self):
        ...
        self.motor_data = {
            ...
            'pos': [deque(maxlen=100) * 4], #enc1, enc2, enc3, enc4
            'vel': [deque(maxlen=100) * 4], # vel1, vel2, vel3, vel4
            'volt': [deque(maxlen=100) * 2], #roboclaw volt1, volt2
            'amp': [deque(maxlen=100) * 4], #roboclaw m1,m2,m3,m4 current draw
        }

        self.imu_data = {
            'time': deque(maxlen=100),
            'lin_accel': [deque(maxlen=100) * 3], #x, y, z acceleration
        }

        self.mocap_data = {
            'time': deque(maxlen=100),
            'pose:': [deque(maxlen=100) * 3], #x, y, z pos
        }
    ...
    def mocap_callback(self, msg):
        ...
        self.mocap_data['pose'][1].append(msg.pose.position.y) #change to linaccel y
        self.mocap_data['pose'][2].append(msg.pose.position.z) #change to linaccel z
